  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="flex w-full items-center gap-2 rounded-md border border-border-default px-3 py-1.5 text-xs text-themed-secondary hover:border-accent/50 hover:text-themed transition-colors"
      >
        <FolderOpen size={14} />
      ...
      {open && (
        <div className="absolute bottom-full left-0 z-50 mb-1 w-72 rounded-md border border-border-default bg-card shadow-xl">
          <div className="max-h-64 overflow-auto p-1">
            {loading && <p className="px-3 py-2 text-xs text-themed-muted">Scanning...</p>}
            {!loading && projects.length === 0 && (
              <p className="px-3 py-2 text-xs text-themed-muted">No projects detected</p>
            )}
            {projects.map((p) => (
              <button
                key={p.path}
                onClick={() => selectProject(p.path)}
                className="flex w-full items-center gap-2 rounded px-3 py-2 text-left text-xs hover:bg-black/5 dark:hover:bg-white/5"
              >
                <FolderOpen size={14} className="shrink-0 text-themed-muted" />
                <div className="min-w-0 flex-1">
                  <div className="truncate font-medium text-themed">{p.name}</div>
                  <div className="truncate text-themed-muted">{p.path}</div>
                </div>
          ...
          <div className="border-t border-border-default p-1">
            <button
              ...
              className="flex w-full items-center gap-2 rounded px-3 py-2 text-xs text-themed-secondary hover:bg-black/5 dark:hover:bg-white/5 hover:text-themed"
            >
